...
export default function GymScanScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    fetch('http://192.168.1.6:8000/api/plans2/')
      .then((res) => res.json())
      .then((data) => setPlans(data))
      .catch((err) => console.error("Помилка при завантаженні планів:", err));
  }, []);
            ...
            <View key={idx} style={styles.exerciseContainer}>
              <View style={styles.exerciseCard}>
                <View style={styles.exerciseTitleBox}>
                  <Text style={styles.exerciseTitleText}>{plan.name}</Text>
                </View>
                <View style={styles.trainingRow}>
                  {plan.exercises.map((exercise, exIdx) => (
                    <Text key={exIdx} style={styles.trainingText}>
...
const styles = StyleSheet.create({
  ...
  logoImage: { width: 45, height: 30, marginRight: 8 },
  logo: { fontSize: 22, fontWeight: 'bold', color: 'black' },
  dateContainer: { alignItems: 'center', textAlign: 'center', justifyContent: 'space-between' },
  dateLabel: { fontSize: 16, textAlign: 'center', marginBottom: 6 },
  dateSelector: {
    flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12,
  },
  dayText: { fontSize: 9, textAlign: 'center', color: '#585555' },
  dayNumber: { fontSize: 13, fontWeight: 'bold', color: '#585555' },
  dateButton: {
    paddingVertical: 6, borderRadius: 12, borderWidth: 1, borderColor: '#5400FD',
    height: 44, width: 40, alignItems: 'center',
  },
  activeDate: {
    paddingVertical: 6, borderRadius: 12, borderWidth: 1, borderColor: '#5400FD',
    backgroundColor: '#DFCBFF', height: 44, width: 40, alignItems: 'center',
  },
  quoteBox: {
    borderWidth: 1, borderColor: '#FDFDFD', backgroundColor: '#fff',
    padding: 30, borderRadius: 16, marginBottom: 20, alignItems: 'center',
    shadowColor: '#000', shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.2, shadowRadius: 6, elevation: 5,
  },
  quoteText: { color: '#35318B', fontSize: 16, fontWeight: '500' },
  activityHeader: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10,
  },
  activityTitle: { fontSize: 20, fontWeight: '400' },
  addButton: {
    width: 25, height: 25, backgroundColor: '#35318B', borderRadius: 20, padding: 6,
  },
  activityList: { gap: 12 },
  exerciseContainer: { flexDirection: 'row', alignItems: 'flex-start' },
  exerciseCard: {
    flex: 1, width: '100%', padding: 10, borderWidth: 1, borderColor: '#FDFDFD',
    backgroundColor: '#fff', paddingHorizontal: 20, paddingVertical: 15,
    borderRadius: 16, marginBottom: 20,
    shadowColor: '#000', shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.2, shadowRadius: 6, elevation: 5,
  },
  exerciseTitleBox: { alignItems: 'center', marginBottom: 6 },
  exerciseTitleText: { fontSize: 16, fontWeight: 600, color: '#000' },
  trainingRow: { alignItems: 'flex-start' },
  trainingText: { fontSize: 14, color: '#000', marginBottom: 4 },
  bottomNav: {
    flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center',
    paddingVertical: 10, borderTopWidth: 1, borderColor: '#ddd', backgroundColor: '#fff',
  },
  navItem: { alignItems: 'center' },
  scanButton: {
    backgroundColor: '#fff', padding: 12, borderRadius: 30, marginTop: -20,
  },
});
